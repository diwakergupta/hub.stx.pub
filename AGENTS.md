...
- **Visualization**: Custom interactive SVG DAG (`CommitDagView`) for block commit graphs, Recharts for charts
...
- **Services**:
    ...
    - `miner-viz.ts`: Generates structured graph data (`MinerVizGraph`) for block commit DAG visualizations.
...
- **Visualizations**:
    - `CommitDagView`: Interactive SVG DAG rendering block commits with cubic Bezier edges, zoom controls, miner filtering, and detail modals.
...
- **Graph Layout**: We use structured blocks and cubic Bezier curves in an SVG canvas. Canonical commits form a vibrant blue spine (`#0284c7`), while fork attempts are styled with dashed red lines (`#ef4444`).
